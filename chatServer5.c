...
int configure_ssl_context(SSL_CTX *ctx, char *name) {
    ...
    else if (strncmp("MovieTalk", name, MAX_TOPIC) == 0) {
        // use the MovieTalk certificate and key
        if (SSL_CTX_use_certificate_file(ctx, "MovieTalk.crt", SSL_FILETYPE_PEM) <= 0) {
            exit(EXIT_FAILURE);
        }
        if (SSL_CTX_use_PrivateKey_file(ctx, "MovieTalk.key", SSL_FILETYPE_PEM) <= 0) {
            ERR_print_errors_fp(stderr);
            exit(EXIT_FAILURE);
        }
        return 0;
...
int main(int argc, char **argv) {
    ...
    if (configure_ssl_context(ctx, current_topic) != 0) {
        printf("error: no key or certificate available for server name.\n");
        exit(EXIT_FAILURE);
    }
        ...
        if (select(max_fd + 1, &readset, NULL, NULL, NULL) > 0) {
            ...
            if (FD_ISSET(server_socket, &readset)) {
                ...
                if (is_nickname_taken(nickname)) {
                    ...
                    SSL_write(ssl, error_msg, strnlen(error_msg, sizeof(error_msg)));
                    SSL_shutdown(ssl);
                    SSL_free(ssl);
                    close(client_sock);
                    continue;
